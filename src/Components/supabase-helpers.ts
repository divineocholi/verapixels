...
export const safeInsert = async <T>(table: string, data: Partial<T>): Promise<T | null> => {
  try {
...
export const safeUpdate = async <T>(table: string, id: string, updates: Partial<T>): Promise<T | null> => {
  try {
